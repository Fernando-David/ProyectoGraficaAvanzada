...
float rotHeliRearX = 0.0f;

// Var animate lambo dor
//int stateDoor = 0;
float dorRotCount = 0.0;
// Variables para la animacion del lambo (Recorrido)
int lamboState = 0;
int numAdvanceLambo = 0;
...
// Variables para la animacion del eclipse
int eclipseState = 0;
float maxAdvance = 0.0; //Avance maximo del coche
int numAdvance = 0;//El eclipse avanza en una direccion
...
float maxHeight =0.0;
bool apagado = false;
float heightCount=10.0;
float velocidadHelice = 0.5f;
bool animacionHeli = true; 
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	modelHeliRear.loadModel("../models/Helicopter/HeliceTrasera.obj");
...
bool processInput(bool continueApplication) {
	...
	if (enableCountSelected && glfwGetKey(window, GLFW_KEY_TAB) == GLFW_PRESS){
		...
		modelSelected++;
		if(modelSelected > 3) //Se se agregan mas se debe de aumentar
			modelSelected = 0;
		if(modelSelected == 1)
	...
	if(modelSelected == 3 && glfwGetKey(window, GLFW_KEY_0) == GLFW_PRESS){
		if(glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_RELEASE){
			rotBuzzHead += 0.1;
		}
		else
	...
	if(modelSelected == 3 && glfwGetKey(window, GLFW_KEY_2) == GLFW_PRESS){
		if(glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_RELEASE){
			rotBuzzLeftArm += 0.01f;
		}
		else
			rotBuzzLeftArm -= 0.01f;
	}
...
void applicationLoop() {
	...
	modelmatrixBuzz = glm::translate(glm::mat4(1.0), glm::vec3(4.0f, 0.0, 22.0));
	...
	while (psi) {
		...
		*******************************************/
		...
		modelMatrixHeliRear = glm::translate(modelMatrixHeliRear, glm::vec3(0, 2.10149f ,-5.64297f));
		modelMatrixHeliRear = glm::rotate(modelMatrixHeliRear, rotHeliRearX, glm::vec3(1, 0, 0));
		modelMatrixHeliRear = glm::translate(modelMatrixHeliRear, glm::vec3(0, -2.10149f ,5.64297f));
		...
		modelMatrixLamboLeftDor = glm::translate(modelMatrixLamboLeftDor, glm::vec3(1.08549,0.699596,0.986726));
		modelMatrixLamboLeftDor = glm::rotate(modelMatrixLamboLeftDor, glm::radians(dorRotCount), glm::vec3(1.0, 0, 0));
		modelMatrixLamboLeftDor = glm::translate(modelMatrixLamboLeftDor, glm::vec3(-1.08549,-0.699596,-0.986726));
		modelLamboLeftDor.render(modelMatrixLamboLeftDor);
		modelLamboRightDor.render(modelMatrixLamboChasis);
		
		//transformaciones para las ruedas del lambo
		glm::mat4 modelMatrixFrontalLeftWheelLambo = glm::mat4(modelMatrixLamboChasis);
		modelMatrixFrontalLeftWheelLambo = glm::translate(modelMatrixFrontalLeftWheelLambo, glm::vec3(0, 0.375f, 1.4f));
		modelMatrixFrontalLeftWheelLambo = glm::rotate(modelMatrixFrontalLeftWheelLambo, rotWheelsYLambo, glm::vec3(0, 1, 0));
		modelMatrixFrontalLeftWheelLambo = glm::rotate(modelMatrixFrontalLeftWheelLambo, rotWheelsXLambo, glm::vec3(1, 0, 0));
		modelMatrixFrontalLeftWheelLambo = glm::translate(modelMatrixFrontalLeftWheelLambo, glm::vec3(0, -0.375f, -1.4f));
		modelLamboFrontLeftWheel.render(modelMatrixFrontalLeftWheelLambo);
		glm::mat4 modelMatrixFrontalRightWheelLambo = glm::mat4(modelMatrixLamboChasis);
		modelMatrixFrontalRightWheelLambo = glm::translate(modelMatrixFrontalRightWheelLambo, glm::vec3(0.0, 0.375f, 1.4f));
		modelMatrixFrontalRightWheelLambo = glm::rotate(modelMatrixFrontalRightWheelLambo, rotWheelsYLambo, glm::vec3(0, 1, 0));
		modelMatrixFrontalRightWheelLambo = glm::rotate(modelMatrixFrontalRightWheelLambo, rotWheelsXLambo, glm::vec3(1, 0, 0));
		modelMatrixFrontalRightWheelLambo = glm::translate(modelMatrixFrontalRightWheelLambo, glm::vec3(0.0, -0.375f, -1.4f));
		modelLamboFrontRightWheel.render(modelMatrixFrontalRightWheelLambo);
		glm::mat4 modelMatrixLamboRearLeftWheel = glm::mat4(modelMatrixLamboChasis);
		modelMatrixLamboRearLeftWheel = glm::translate(modelMatrixLamboRearLeftWheel, glm::vec3(0.0, 0.4f, -1.6f));
		...
		modelMatrixLamboRearRightWheel = glm::translate(modelMatrixLamboRearRightWheel, glm::vec3(0.0, 0.4f, -1.6f));
		...
		modelMatrixTorso = glm::scale(modelMatrixTorso, glm::vec3(4.0));
		...
		modelMatrixLeftArm = glm::translate(modelMatrixLeftArm, glm::vec3(0.151378f, 0.580236f, -0.019438f));
		modelMatrixLeftArm = glm::rotate(modelMatrixLeftArm, glm::radians(-60.0f), glm::vec3(0,0,1));
		...
		if(record && modelSelected == 1){
			...
			numPasosDartJoints++;
			if(interpolationDartJoints > 1.0){// Ya se llegó al siguiente frame
		...
		if(record && modelSelected == 2){ // Para toda la matrix del dart
		...
		//Maquina de estado para el eclipse
		switch (eclipseState)
		{
		case 0:
			if(numAdvance == 0){
			...
			else if(numAdvance == 1){
				maxAdvance = 49.0f;
			}
			else if (numAdvance == 2){
				maxAdvance = 48.0f;
			}
			else if (numAdvance == 3){
				maxAdvance = 45.0f;
			}
			...
			break;
		case 1:
			modelMatrixEclipse = glm::translate(modelMatrixEclipse, glm::vec3(0, 0, 0.1f));
			rotWheelsX += 0.1;
			advanceCount += 0.1f;
			rotWheelsY -= 0.01f;
			if(rotWheelsY < 0)
		...
		case 2:
			modelMatrixEclipse = glm::translate(modelMatrixEclipse, glm::vec3(0, 0, 0.1f));
			modelMatrixEclipse = glm::rotate(modelMatrixEclipse, glm::radians(1.5f), glm::vec3(0, 1, 0));
			rotWheelsY += 0.01;
			rotCount += 1.5f;
			if(rotWheelsY > 2.7){
				rotWheelsY = 2.7;
		...
		//Maquina de estado para el lambo
		switch (lamboState)
		{
		case 0:
			if(numAdvanceLambo == 0){
				maxAdvanceLambo = 10.0f;
				lamboState = 1;
			}
			else if(numAdvanceLambo == 1){
				maxAdvanceLambo = 42.0f;
				lamboState = 1;
			}
			else if (numAdvanceLambo == 2){
				maxAdvanceLambo = 40.0f;
				lamboState = 1;
			}
			else if (numAdvanceLambo == 3){
				maxAdvanceLambo = -58.0f;
				lamboState = 3;
			}
			
			break;
		case 1:
			modelMatrixLambo = glm::translate(modelMatrixLambo, glm::vec3(0, 0, 0.1f));
			rotWheelsXLambo += 0.1;
			advanceCountLambo += 0.1f;
			rotWheelsYLambo += 0.01f;
			if(rotWheelsYLambo > 0)
				rotWheelsYLambo = 0;
			if(advanceCountLambo >= maxAdvanceLambo){
				advanceCountLambo = 0;
				lamboState = 2;
				if(numAdvanceLambo == 2){
					lamboState = 0;
					numAdvanceLambo = 3;
				}
			}
		...
		case 2:
			modelMatrixLambo = glm::translate(modelMatrixLambo, glm::vec3(0, 0, 0.1f));
			modelMatrixLambo = glm::rotate(modelMatrixLambo, glm::radians(-1.5f), glm::vec3(0, 1, 0));
			rotWheelsYLambo -= 0.01;
			rotCountLambo += 1.5f;
			if(rotWheelsYLambo < -2.7){
				rotWheelsYLambo = -2.7;
			}
		...
		//Maquina de estados para el Helicoportero
		switch (heliState)
		{
		case 0: 
			if(apagado == true){//Descenso
				maxHeight = 0.0f;
				heliState = 1;
			}
			else if(apagado == false){//Ascenso
				maxHeight = 10.0f;
				heliState = 2;
			}
			break;
		case 1:
			modelMatrixHeli = glm::translate(modelMatrixHeli, glm::vec3(0, -0.03f, 0));
			heightCount -= 0.03f;
		...
		//Encendido y apagado de la animacion del helicoptero.
		if(animacionHeli && glfwGetKey(window, GLFW_KEY_H) == GLFW_PRESS){
			animacionHeli = false;
			apagado = !apagado;
			heliState = 0;
		...
		if(apagado == false){
			rotHelHelY += velocidadHelice;
			rotHeliRearX += velocidadHelice;
			if(velocidadHelice < 0.5){
				velocidadHelice += 0.01;
			}
		}else{
			rotHelHelY += velocidadHelice;
			rotHeliRearX += velocidadHelice;

			if(velocidadHelice > 0){
				velocidadHelice -= 0.001;
				if (velocidadHelice < 0) velocidadHelice = 0;
			}
