...
bool testSLABPlane(float p, float v, float min, float max, float &tmin, float &tmax){
	...
	float t1 = (min - p) / v;
	float t2 = (max - p) / v;
	if (t1 > t2) { //swap del valor mas pequeño
		float temp = t1;
		t1 = t2;
		t2 = temp;
	}
...
bool instersectSegmentAABB(glm::vec3 o, glm::vec3 t, AbstractModel::AABB collider){
	float tmin = -FLT_MAX;
	float tmax = FLT_MAX;
...
bool testRayOBB(glm::vec3 o, glm::vec3 d, AbstractModel::OBB colliderOBB) {
	// 1.- Obtener el quaternion inverso de la irientacion de la caja
	glm::quat qInv = glm::inverse(colliderOBB.u);// u es la orientacion de la caja
	// 2.- Transformar el origen y destino del rayo en realcion a la inversa
	glm::vec3 oTemp = qInv * o; //origne del rayo en el espacio de la caja
	glm::vec3 dTemp = qInv * d; //destino del rayo en el espacio de la caja
	glm:: vec3 cOBBTemp = qInv * colliderOBB.c; //centro de la caja en el espacio de la caja
	// 3.- crear la AABB
	AbstractModel::AABB aabb;
	aabb.mins = cOBBTemp - colliderOBB.e;// e es la media longitud de la caja
	aabb.maxs = cOBBTemp + colliderOBB.e;
	// 4.- Aplicar la prueba de colision entre AABB y Rayo
	return instersectSegmentAABB(oTemp, dTemp, aabb);
}
