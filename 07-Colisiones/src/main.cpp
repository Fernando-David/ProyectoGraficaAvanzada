...
Box boxCollider;
Sphere esferaCollider(10, 10);
Cylinder cylinderCollider(10, 10, 1, 1, 1);
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	esferaCollider.init();
	esferaCollider.setShader(&shader);
	boxCollider.init();
...
bool processInput(bool continueApplication) {
	...
	{
		std::cout << "Se detecta el joystick" << std::endl;
		int axesCount;
		const float * axes = glfwGetJoystickAxes(GLFW_JOYSTICK_1, &axesCount);
		//Descomentar lo siguiente para depurar la entrada de los potenciometros (joysticks y gatillos)
		/*
		std::cout << "El numero de axes es:" << axesCount << std::endl;
		std::cout << "El eje x del joy izquierdo:  " << axes[0] << std::endl; //Horizontal Izq
		std::cout << "El eje y del joy izquierdo:  " << axes[1] << std::endl; //Vert Izq
		std::cout << "El eje x del joy derecho:  " << axes[2] << std::endl; //Horizontal Der
		std::cout << "El eje y del joy derecho:  " << axes[3] << std::endl; //Vertical Der
		std::cout << "El gatillo izquierdo L2:  " << axes[4] << std::endl; //Gatillo izq (-1 a 1)
		std::cout << "El gatillo derecho R2:  " << axes[5] << std::endl; //Gatillo der  (-1 a 1)
		*/
		if(fabs(axes[0]) >= 0.2){
			modelMatrixMayow = glm::rotate(modelMatrixMayow, -axes[0]*0.02f, glm::vec3(0,1,0));
			animationMayowIndex = 0;
		}
		if (fabs(axes[1]) >= 0.2){
			modelMatrixMayow = glm::translate(modelMatrixMayow, glm::vec3(0,0,-axes[1] * 0.08f));
			animationMayowIndex = 0;
		}
		if (fabs(axes[2]) >= 0.2){
			camera->mouseMoveCamera(axes[2]*0.8, 0, deltaTime);
		}
		if (fabs(axes[3]) >= 0.2){
			camera->mouseMoveCamera(0, axes[3]*0.8, deltaTime);
		}
		int numeroBotones;
		...
		if (botones[0] == GLFW_PRESS && !isJump){ // Boton A
			startTime = currTime;
			isJump = true;
			tmV = 0;
		}
...
void applicationLoop() {
	...
	while (psi) {
		...
		if(!mayStopJump){
			//modelMatrixMayow[3].y = terrain.getHeightTerrain(modelMatrixMayow[3][0], modelMatrixMayow[3][2]);
			modelMatrixMayow[3][1] = gravity * tmV * tmV + 4.0 * tmV + terrain.getHeightTerrain(modelMatrixMayow[3][0], modelMatrixMayow[3][2]);
		...
		//se agregan modelos a la colision
		glm::mat4 modelColliderRock = glm::mat4(matrixModelRock);
		AbstractModel::SBB colliderRock;
		modelColliderRock = glm::scale(modelColliderRock, glm::vec3(1.0));
		modelColliderRock = glm::translate(modelColliderRock, modelRock.getSbb().c);
		colliderRock.c = modelColliderRock[3];
		colliderRock.ratio = modelRock.getSbb().ratio * 1.0f; //Este valor es el escalamiento que puede tener el modelo.
		addOrUpdateColliders(collidersSBB, "rock", colliderRock, modelColliderRock);

		//Aircraft
		glm::mat4 modelColliderAircraft = glm::mat4(modelMatrixAircraft);//Matriz copia
		AbstractModel::OBB colliderAircraft;
		colliderAircraft.u = glm::quat_cast(modelColliderAircraft);//Matriz 4*4 a quaternion
		//Colocar escalamiento que tenia el objeto
		modelColliderAircraft = glm::scale(modelColliderAircraft, glm::vec3(1.0));
		//Traslacion al centro de la caja
		modelColliderAircraft = glm::translate(modelColliderAircraft, modelAircraft.getObb().c);//Se obtiene el centro del modelo y se traslada la matriz a ese punto
		colliderAircraft.c = modelColliderAircraft[3];//Se obtiene la traslacion de la matriz para colocar el centro del collider
		colliderAircraft.e = modelAircraft.getObb().e * glm::vec3(1.0f);//Se obtiene el escalamiento del collider
		addOrUpdateColliders(collidersOBB, "aircraft", colliderAircraft, modelColliderAircraft);

		//May
		glm::mat4 modelColliderMay = glm::mat4(modelMatrixMayow);
		AbstractModel::OBB colliderMay;
		modelColliderMay = glm::rotate(modelColliderMay, glm::radians(-90.0f), glm::vec3(1, 0, 0));
		colliderMay.u = glm::quat_cast(modelColliderMay);
		modelColliderMay = glm::scale(modelColliderMay, glm::vec3(0.021f));
		modelColliderMay = glm::translate(modelColliderMay, mayowModelAnimate.getObb().c);
		colliderMay.c = modelColliderMay[3];
		colliderMay.e = mayowModelAnimate.getObb().e * glm::vec3(0.021f) * glm::vec3(0.78f, 0.78f, 0.78f);
		addOrUpdateColliders(collidersOBB, "may", colliderMay, modelColliderMay);

		//Dart
		glm::mat4 modelColliderDart = glm::mat4(modelMatrixDart);
		AbstractModel::OBB colliderDart;
		colliderDart.u = glm::quat_cast(modelColliderDart);
		modelColliderDart = glm::scale(modelColliderDart, glm::vec3(0.5f));
		modelColliderDart = glm::translate(modelColliderDart, modelDartLegoBody.getObb().c);
		colliderDart.c = modelColliderDart[3];
		colliderDart.e = modelDartLegoBody.getObb().e * glm::vec3(0.5f);
		addOrUpdateColliders(collidersOBB, "dart", colliderDart, modelMatrixDart);

		for(int i = 0; i < lamp1Position.size(); i++){
			AbstractModel::OBB modelCollider;
			glm::mat4 matrixCollider = glm::mat4(1.0);
			matrixCollider = glm::translate(matrixCollider, lamp1Position[i]);
			matrixCollider = glm::rotate(matrixCollider, 
				glm::radians(lamp1Orientation[i]), glm::vec3(0, 1, 0));
			addOrUpdateColliders(collidersOBB, "lamp1_" + std::to_string(i), 
				modelCollider, matrixCollider);
			modelCollider.u = glm::quat_cast(matrixCollider);
			matrixCollider = glm::scale(matrixCollider, glm::vec3(0.5f));//Mismo scale del modelo
			matrixCollider = glm::translate(matrixCollider, modelLamp1.getObb().c);
			modelCollider.c = matrixCollider[3];
			modelCollider.e = modelLamp1.getObb().e * glm::vec3(0.5f);//Mismo scale del modelo
			std::get<0>(collidersOBB.find("lamp1_" + std::to_string(i))->second) = modelCollider;
		}

		//reder de los colliders
		for(auto it = collidersSBB.begin(); it != collidersSBB.end(); it++){
			...
			esferaCollider.setColor(glm::vec4(1, 1, 1,1));
			esferaCollider.enableWireMode();
			esferaCollider.render(matrixCollider);
		...
		for(auto it = collidersOBB.begin(); it != collidersOBB.end(); it++){
			...
			for (auto jt = collidersOBB.begin(); jt != collidersOBB.end(); jt++){
				if(it != jt && testOBBOBB(std::get<0>(it->second), std::get<0>(jt->second))){
					isCollision = true;
					std::cout << "Colision del modelo " << it->first 
						<< "y el modelo " << jt->first << std::endl;
					
				}
			}
			// Actualizar banderas que indican colision
		...
		for(auto it = collidersOBB.begin(); it != collidersOBB.end(); it++){
			...
			for (auto jt = collidersSBB.begin(); jt != collidersSBB.end(); jt++){
				if(testSphereOBox(std::get<0>(jt->second), std::get<0>(it->second))){
					std::cout << "Hay colision entre el modelo " << it->first 
						<< "y el modelo " << jt->first << std::endl;
					std::cout << "Hay colision entre el modelo " << jt->first 
						<< "y el modelo " << it->first << std::endl;
					isCollision = true;
		...
		for(auto it = collidersSBB.begin(); it != collidersSBB.end(); it++){
			...
			for (auto jt = collidersSBB.begin(); jt != collidersSBB.end(); jt++){
				if(it != jt && testSphereSphereIntersection(
					...
					std::cout << "Colision entre la esfera  " << it->first 
						<< "y el modelo " << jt->first << std::endl;
					isCollision = true;
				}
			}
		...
		auto collision = collisionDetection.begin();
		bool stopMotionDart = false;
		for( ;collision != collisionDetection.end(); collision++){
			auto modeloOBB = collidersOBB.find(collision->first);
			auto modeloSBB = collidersSBB.find(collision->first);
			if(modeloSBB != collidersSBB.end()){
				if(!collision->second){
					addOrUpdateColliders(collidersSBB, collision->first);
				}
				else{}//En caso de tener objetos con collider esferico
			}
			else if(modeloOBB != collidersOBB.end()){
				if(!collision->second){
					addOrUpdateColliders(collidersOBB, collision->first);
					/*if(collision->first.compare("may") == 0)
						mayStopJump = true;*/
				}
				else{ 
				if(collision->first.compare("may") == 0){
						modelMatrixMayow = std::get<1>(modeloOBB->second);
						mayStopJump = true;
					}else{
						mayStopJump = false;
					}
					if(collision->first.compare("dart") == 0){
						stopMotionDart = true;
					}
				}
			}
		...
		// Picking por un rayo
		//1. Desde un modelo
		glm::mat4 matrixRay = glm::mat4(modelMatrixMayow);
		matrixRay = glm::translate(matrixRay, glm::vec3(0, 1.0, 0));
		float maxDistanceRay = 10;
		glm::vec3 rayDirection = matrixRay[2];
		glm:: vec3 rayOrigen = matrixRay[3];
		glm::vec3 rayPM = rayOrigen + rayDirection * maxDistanceRay / 2.0f;
		glm::vec3 rayTarge = rayOrigen + rayDirection * maxDistanceRay;
		matrixRay[3] = glm::vec4 (rayPM, 1.0);
		matrixRay =glm::rotate(matrixRay, glm::radians(90.0f), glm::vec3(1, 0, 0));
		matrixRay = glm::scale(matrixRay, glm::vec3(0.1, maxDistanceRay, 0.1));
		cylinderCollider.setColor(glm::vec4(1.0));
		cylinderCollider.render(matrixRay);

		for(auto it = collidersSBB.begin(); it != collidersSBB.end(); it++){
			float t;
			if(raySphereIntersect(rayOrigen, rayTarge, rayDirection, std::get<0>(it->second), t)){
				std::cout << "El rayo colisiona con el modelo " << it->first << std::endl;
			}
		}

		glm::vec3 od = glm::unProject(glm::vec3(lastMousePosX, screenHeight - lastMousePosY,
			 0.0), view, projection, glm::vec4(0, 0, screenWidth, screenHeight));
		glm::vec3 td = glm::unProject(glm::vec3(lastMousePosX, screenHeight - lastMousePosY,
			 1.0), view, projection, glm::vec4(0, 0, screenWidth, screenHeight));
		glm::vec3 dir = glm::normalize(td - od);

		for(auto it = collidersSBB.begin(); it != collidersSBB.end(); it++){
			float t;
			if(raySphereIntersect(od, td, dir, std::get<0>(it->second), t)){
				std::cout << "Seleccionando el modelo => " << it->first << std::endl;
			}
		}
		for(auto it = collidersOBB.begin(); it != collidersOBB.end(); it++){ // Sirve para el mouse picking
			if(testRayOBB(od, td, std::get<0>(it->second))){
				std::cout << "Seleccionando el modelo => " << it->first << std::endl;
			}
			if(testRayOBB(rayOrigen, rayTarge, std::get<0>(it->second))){
				std::cout << "Seleccionando desde el modelo de may => " << it->first << std::endl;
			}
		}
		...
		else if(keyFramesDart.size() > 0 && !stopMotionDart){
