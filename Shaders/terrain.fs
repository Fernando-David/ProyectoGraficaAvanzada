...
vec3 calculateDirectionalLight(Light light, vec3 direction){
	...
	float principalTextureCantidad = 1 - (blendMapColor.r + blendMapColor.g + blendMapColor.b);
	...
	vec4 totalColor = principalTextureColor + rTextureColor + gTextureColor + bTextureColor;
