...
Terrain terrain(-1, -1, 200, 32, "../Textures/renderMapa.png");
...
//variables para salto de spiderman
bool isSpidermanJumping = false;
float spidermanJumpTimer = 0.0f;
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	// Spiderman
	spidermanModelAnimate.loadModel("../models/Spiderman/spiModelo.fbx");
	...
	Texture textureR("../Textures/Tundra.jpg"); //Textura de la tundra
	...
	// Definiendo la textura
	//Texture textureG("../Textures/water2.jpg");
	Texture textureG("../Textures/FloresBlancas.png"); //Textura de flores para el pasto
	...
	Texture textureB("../Textures/Nieve.png");//Textura de nieve
	...
	// Definiendo la textura
	Texture textureBlendMap("../Textures/RenderBlendMap.png");//Nuevo blend map
...
bool processInput(bool continueApplication) {
	...
	// Controles de spiderman
	if (modelSelected == 5) {
		//Detectar el salto
		if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS && !isSpidermanJumping) {
			isSpidermanJumping = true;
			spidermanJumpTimer = 0.0f;
		}
		...
		if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS){
			modelMatrixSpiderman = glm::translate(modelMatrixSpiderman, glm::vec3(0.0, 0.0, 0.08));
			isMoving = true;
		}
		...
		// Animacion
		if (!isSpidermanJumping) {
			if (isMoving) {
				animationSpidermanIndex = 1; // CORRER
			} else {
				animationSpidermanIndex = 0; // ESTAR (Idle)
			}
		}
	}
	glfwPollEvents();
...
void applicationLoop() {
	...
	//ubicacion del modelo de spiderman
	modelMatrixSpiderman = glm::translate(modelMatrixSpiderman, glm::vec3(10.0f, 0.05f, 5.0f));
	modelMatrixSpiderman = glm::rotate(modelMatrixSpiderman, glm::radians(90.0f), glm::vec3(1.0, 0.0, 0.0));
	//modelMatrixSpiderman = glm::rotate(modelMatrixSpiderman, glm::radians(90.0f), glm::vec3(0.0, 1.0, 0.0));
	...
	while (psi) {
		...
		shaderTerreno.setMatrix4("projection", 1, false,
				glm::value_ptr(projection));
		shaderTerreno.setMatrix4("view", 1, false,
				glm::value_ptr(view));
		...
		shaderTerreno.setVectorFloat3("viewPos", glm::value_ptr(camera->getPosition()));
		shaderTerreno.setVectorFloat3("directionalLight.light.ambient", glm::value_ptr(glm::vec3(0.6, 0.6, 0.6)));
		shaderTerreno.setVectorFloat3("directionalLight.light.diffuse", glm::value_ptr(glm::vec3(0.7, 0.7, 0.7)));
		shaderTerreno.setVectorFloat3("directionalLight.light.specular", glm::value_ptr(glm::vec3(0.9, 0.9, 0.9)));
		shaderTerreno.setVectorFloat3("directionalLight.direction", glm::value_ptr(glm::vec3(-1.0, 0.0, 0.0)));

		/*******************************************
		 * Propiedades SpotLights
		 *******************************************/
		shaderMulLighting.setInt("spotLightCount", 0);
		shaderTerreno.setInt("spotLightCount", 0); //Se agregó para el terreno

		/*******************************************
		 * Propiedades PointLights
		 *******************************************/
		shaderMulLighting.setInt("pointLightCount", 0);
		shaderTerreno.setInt("pointLightCount", 0); //Se agregó para el terreno
		...
		shaderTerreno.setInt("blendMapTexture", 1);
		glActiveTexture(GL_TEXTURE3);
		glBindTexture(GL_TEXTURE_2D, TextureRID);
		shaderTerreno.setInt("rTexture", 3);
		glActiveTexture(GL_TEXTURE4);
		glBindTexture(GL_TEXTURE_2D, TextureGID);
		shaderTerreno.setInt("gTexture", 4);
		glActiveTexture(GL_TEXTURE5);
		glBindTexture(GL_TEXTURE_2D, TextureBID);
		shaderTerreno.setInt("bTexture", 5);
		shaderTerreno.setVectorFloat2("scaleUV", glm::value_ptr(glm::vec2(80, 80)));
		terrain.setPosition(glm::vec3(100, 0, 100));
		terrain.render();
		shaderTerreno.setVectorFloat2("scaleUV", glm::value_ptr(glm::vec2(0, 0)));
		glBindTexture(GL_TEXTURE_2D, 0);
		...
		glm::vec4 leftWheelFront = glm::translate(modelMatrixEclipse, glm::vec3(2.4588f, 0, 4.11483))[3];
		glm::vec4 rightWheelFront = glm::translate(modelMatrixEclipse, glm::vec3(-2.4588f, 0, 4.11483))[3];
		glm::vec4 leftWheelRear = glm::translate(modelMatrixEclipse, glm::vec3(2.4588f, 0, -4.372))[3];
		glm::vec4 rightWheelRear = glm::translate(modelMatrixEclipse, glm::vec3(-2.4588f, 0, -4.372))[3];
		leftWheelFront.y = terrain.getHeightTerrain(leftWheelFront.x, leftWheelFront.z);
		rightWheelFront.y = terrain.getHeightTerrain(rightWheelFront.x, rightWheelFront.z);
		leftWheelRear.y = terrain.getHeightTerrain(leftWheelRear.x, leftWheelRear.z);
		rightWheelRear.y = terrain.getHeightTerrain(rightWheelRear.x, rightWheelRear.z);
		glm::vec3 uVec = leftWheelFront - leftWheelRear;
		glm::vec3 vVec = rightWheelRear - leftWheelRear;
		glm::vec4 ejeYEclipse = glm::vec4(
			 glm::cross(glm::normalize(vVec), glm::normalize(uVec)),0);
		glm::vec4 ejeXEclipse = modelMatrixEclipse[0]; //Temporal
		glm::vec4 ejeZEclipse = glm::vec4( 
			glm::normalize(glm::cross(glm::vec3(ejeXEclipse), glm::vec3(ejeYEclipse))),0);
		ejeXEclipse = glm::vec4(
			glm::normalize(glm::cross(glm::vec3(ejeYEclipse), glm::vec3(ejeZEclipse))),0);// Se actualiza el eje x
		modelMatrixEclipse[0] = ejeXEclipse;
		modelMatrixEclipse[1] = ejeYEclipse;
		modelMatrixEclipse[2] = ejeZEclipse;
		...
		// Spiderman
		modelMatrixSpiderman[3].y = terrain.getHeightTerrain(modelMatrixSpiderman[3].x, modelMatrixSpiderman[3].z);
		glm::vec3 upModelSpidey = glm::normalize(
		 terrain.getNormalTerrain(modelMatrixSpiderman[3].x, modelMatrixSpiderman[3].z)
		);
		glm::vec3 rightModelSpidey = glm::normalize(modelMatrixSpiderman[0]);
		glm::vec3 frontModelSpidey = glm::normalize(glm::cross(rightModelSpidey, upModelSpidey));
		rightModelSpidey = glm::normalize(glm::cross(upModelSpidey, frontModelSpidey));
		modelMatrixSpiderman[0] = glm::vec4(rightModelSpidey, 0);
		modelMatrixSpiderman[1] = glm::vec4(upModelSpidey, 0);
		modelMatrixSpiderman[2] = glm::vec4(frontModelSpidey, 0);
		glm::mat4 modelMatrixSpidermanBody = glm::mat4(modelMatrixSpiderman);
		modelMatrixSpidermanBody = glm::scale(modelMatrixSpidermanBody, glm::vec3(0.021f)); 
		//Temporizador de salto
		if (isSpidermanJumping) {
			animationSpidermanIndex = 2;
			spidermanJumpTimer += deltaTime;
			if (spidermanJumpTimer >= 1.83333f) { 
				isSpidermanJumping = false;
			}
		}
