...
//Terrain terrain(-1, -1, 200, 32, "../Textures/heightmap2026-2.png");
...
*/
std::string fileNames[6] = { "../Textures/cloudy/bluecloud_ft.tga",
		"../Textures/cloudy/bluecloud_bk.tga",
		"../Textures/cloudy/bluecloud_up.tga",
		"../Textures/cloudy/bluecloud_dn.tga",
		"../Textures/cloudy/bluecloud_rt.tga",
		"../Textures/cloudy/bluecloud_lf.tga" };
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	// Spiderman
	spidermanModelAnimate.loadModel("../models/Spiderman/spi.fbx");
...
bool processInput(bool continueApplication) {
	...
	if (modelSelected == 5 && glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS){
		modelMatrixSpiderman = glm::translate(modelMatrixSpiderman, glm::vec3(0.0, 0.0, 0.05));
		animationSpidermanIndex = 1;
	}
...
void applicationLoop() {
	...
	while (psi) {
		...
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, textureCespedID);
		terrain.setPosition(glm::vec3(100, 0, 100));//Se recomienda que se ponga a la mitad de ñas dimensiones del terreno para que se vea mejor
		shaderMulLighting.setVectorFloat2("scaleUV", glm::value_ptr(glm::vec2(250, 250)));//Escalar la textura del cesped
		//terrain.enableWireMode();
		terrain.render();
		shaderMulLighting.setVectorFloat2("scaleUV", glm::value_ptr(glm::vec2(0, 0)));//Para regresar a la escala original de la textura
		glBindTexture(GL_TEXTURE_2D, 0);
		 ...
		 * **************************************/
		modelMatrixMayow[3].y = terrain.getHeightTerrain(modelMatrixMayow[3].x, modelMatrixMayow[3].z);//ajuste de la altura del personaje con el terreno
		...
		glm::vec3 frontModel = glm::normalize(glm::cross(rightModel, upModel));
		rightModel = glm::normalize(glm::cross(upModel, frontModel));
		modelMatrixMayow[0] = glm::vec4(rightModel, 0);
		modelMatrixMayow[1] = glm::vec4(upModel, 0);
		modelMatrixMayow[2] = glm::vec4(frontModel, 0);
		...
		modelMatrixCowboy[3].y = terrain.getHeightTerrain(modelMatrixCowboy[3].x, modelMatrixCowboy[3].z);//ajuste de la altura del personaje con el terreno
		...
		modelMatrixGuardian[3].y = terrain.getHeightTerrain(modelMatrixGuardian[3].x, modelMatrixGuardian[3].z);//ajuste de la altura del personaje con el terreno
		...
		modelMatrixCyborg[3].y = terrain.getHeightTerrain(modelMatrixCyborg[3].x, modelMatrixCyborg[3].z);//ajuste de la altura del personaje con el terreno
