...
std::vector<std::pair<glm::vec3, float>> lamp1Position = {
	{glm::vec3(-7, 0, -19.4), -17},
	{glm::vec3(24.4, 0, -32.57), -82},
	{glm::vec3(-10.15, 0, -54.1), -23}
};

std::vector<std::pair<glm::vec3, float>> lamp2Position = {
	{glm::vec3(-36.52, 0, -23.24), 21.37 + 90},
	{glm::vec3(-52.73, 0, -3.90), -65 + 90},
	{glm::vec3(22.65, 0, -2.14), 154.0}
};

std::vector<std::pair<glm::vec3, float>> lamp3Position = {
	{glm::vec3(15.0, 0, -15.0), 0.0},
	{glm::vec3(-15.0, 0, 15.0), 45.0} 
};
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	modelLamp3.loadModel("../models/Antorcha/Antorcha.fbx");
...
bool processInput(bool continueApplication) {
	...
	if (enableCountSelected && glfwGetKey(window, GLFW_KEY_TAB) == GLFW_PRESS){
		...
		if(modelSelected == 5)
        	std::cout << "Modelo seleccionado: Lamborghini" << std::endl;
...
void applicationLoop() {
	...
	float maxAdvanceLambo = 0.0;
	const float avanceLambo = 0.15f; // Velocidad del lambo
	const float giroLambo = 1.0f;    // Velocidad de giro
	...
	while (psi) {
		...
		shaderTerrain.setVectorFloat3("directionalLight.light.diffuse", glm::value_ptr(glm::vec3(0.4, 0.4, 0.4)));
		...
		glm::vec3 spotPosition = modelMatrixHeli * glm::vec4(0.0, 0.2, 1.67, 1.0);
		shaderMulLighting.setVectorFloat3(
			"spotLights[0].position", 
			glm::value_ptr(glm::vec3(spotPosition)));
		shaderMulLighting.setVectorFloat3(
		...
		shaderMulLighting.setVectorFloat3(
			...
			glm::value_ptr(glm::vec3(0.7, 0.68, 0.03)));
		shaderMulLighting.setFloat(
			"spotLights[0].constant", 1.0f);
		shaderMulLighting.setFloat(
			"spotLights[0].linear", 0.005f);
		shaderMulLighting.setFloat(
			"spotLights[0].quadratic", 0.002f);
		shaderMulLighting.setFloat(
		...
		// FAROS DEL LAMBORGHINI
		// 1. Obtenemos el vector "hacia adelante" multiplicando la matriz del carro por Z positivo
		glm::vec3 lamboForward = glm::normalize(glm::vec3(modelMatrixLambo * glm::vec4(0.0, 0.0, 1.0, 0.0)));

		// 2. Definimos las posiciones locales de los faros izquierdo y derecho.
		// IMPORTANTE: Juega con estos 3 números (X, Y, Z) para que la luz salga exactamente del faro del modelo.
		glm::vec3 leftLamboLight = glm::vec3(modelMatrixLambo * glm::vec4(1.0, 0.9, 2.5, 1.0)); 
		glm::vec3 rightLamboLight = glm::vec3(modelMatrixLambo * glm::vec4(-1.0, 0.9, 2.5, 1.0));

		// --- Faro Izquierdo (SpotLight 1) ---
		// Para shaderMulLighting
		shaderMulLighting.setVectorFloat3("spotLights[1].position", glm::value_ptr(leftLamboLight));
		shaderMulLighting.setVectorFloat3("spotLights[1].direction", glm::value_ptr(lamboForward));
		shaderMulLighting.setVectorFloat3("spotLights[1].light.ambient", glm::value_ptr(glm::vec3(0.5, 0.5, 0.5)));
		shaderMulLighting.setVectorFloat3("spotLights[1].light.diffuse", glm::value_ptr(glm::vec3(0.9, 0.9, 0.9)));
		shaderMulLighting.setVectorFloat3("spotLights[1].light.specular", glm::value_ptr(glm::vec3(1.0, 1.0, 1.0)));
		shaderMulLighting.setFloat("spotLights[1].constant", 1.0f);
		shaderMulLighting.setFloat("spotLights[1].linear", 0.014f);
		shaderMulLighting.setFloat("spotLights[1].quadratic", 0.0007f);
		shaderMulLighting.setFloat("spotLights[1].cutOff", glm::cos(glm::radians(15.0f)));
		shaderMulLighting.setFloat("spotLights[1].outerCutOff", glm::cos(glm::radians(25.0f)));

		// Para shaderTerrain (misma luz, afecta al pasto)
		shaderTerrain.setVectorFloat3("spotLights[1].position", glm::value_ptr(leftLamboLight));
		shaderTerrain.setVectorFloat3("spotLights[1].direction", glm::value_ptr(lamboForward));
		shaderTerrain.setVectorFloat3("spotLights[1].light.ambient", glm::value_ptr(glm::vec3(0.5, 0.5, 0.5)));
		shaderTerrain.setVectorFloat3("spotLights[1].light.diffuse", glm::value_ptr(glm::vec3(0.9, 0.9, 0.9)));
		shaderTerrain.setVectorFloat3("spotLights[1].light.specular", glm::value_ptr(glm::vec3(1.0, 1.0, 1.0)));
		shaderTerrain.setFloat("spotLights[1].constant", 1.0f);
		shaderTerrain.setFloat("spotLights[1].linear", 0.014f);
		shaderTerrain.setFloat("spotLights[1].quadratic", 0.0007f);
		shaderTerrain.setFloat("spotLights[1].cutOff", glm::cos(glm::radians(15.0f)));
		shaderTerrain.setFloat("spotLights[1].outerCutOff", glm::cos(glm::radians(25.0f)));

		// --- Faro Derecho (SpotLight 2) ---
		// Para shaderMulLighting
		shaderMulLighting.setVectorFloat3("spotLights[2].position", glm::value_ptr(rightLamboLight));
		shaderMulLighting.setVectorFloat3("spotLights[2].direction", glm::value_ptr(lamboForward));
		shaderMulLighting.setVectorFloat3("spotLights[2].light.ambient", glm::value_ptr(glm::vec3(0.5, 0.5, 0.5)));
		shaderMulLighting.setVectorFloat3("spotLights[2].light.diffuse", glm::value_ptr(glm::vec3(0.9, 0.9, 0.9)));
		shaderMulLighting.setVectorFloat3("spotLights[2].light.specular", glm::value_ptr(glm::vec3(1.0, 1.0, 1.0)));
		shaderMulLighting.setFloat("spotLights[2].constant", 1.0f);
		shaderMulLighting.setFloat("spotLights[2].linear", 0.014f);
		shaderMulLighting.setFloat("spotLights[2].quadratic", 0.0007f);
		shaderMulLighting.setFloat("spotLights[2].cutOff", glm::cos(glm::radians(15.0f)));
		shaderMulLighting.setFloat("spotLights[2].outerCutOff", glm::cos(glm::radians(25.0f)));

		// Para shaderTerrain
		shaderTerrain.setVectorFloat3("spotLights[2].position", glm::value_ptr(rightLamboLight));
		shaderTerrain.setVectorFloat3("spotLights[2].direction", glm::value_ptr(lamboForward));
		shaderTerrain.setVectorFloat3("spotLights[2].light.ambient", glm::value_ptr(glm::vec3(0.5, 0.5, 0.5)));
		shaderTerrain.setVectorFloat3("spotLights[2].light.diffuse", glm::value_ptr(glm::vec3(0.9, 0.9, 0.9)));
		shaderTerrain.setVectorFloat3("spotLights[2].light.specular", glm::value_ptr(glm::vec3(1.0, 1.0, 1.0)));
		shaderTerrain.setFloat("spotLights[2].constant", 1.0f);
		shaderTerrain.setFloat("spotLights[2].linear", 0.014f);
		shaderTerrain.setFloat("spotLights[2].quadratic", 0.0007f);
		shaderTerrain.setFloat("spotLights[2].cutOff", glm::cos(glm::radians(15.0f)));
		shaderTerrain.setFloat("spotLights[2].outerCutOff", glm::cos(glm::radians(25.0f)));
		...
		int totalPointLights = lamp1Position.size() + lamp2Position.size() + lamp3Position.size();
		shaderMulLighting.setInt("pointLightCount", totalPointLights);
		shaderTerrain.setInt("pointLightCount", totalPointLights);

		for (int i = 0; i < lamp1Position.size(); i++){
			glm::mat4 modelMatrixLamp = glm::mat4(1.0);
			modelMatrixLamp = glm::translate(modelMatrixLamp, lamp1Position[i].first);
			modelMatrixLamp = glm::rotate(
				modelMatrixLamp, glm::radians(lamp1Position[i].second), glm::vec3(0, 1, 0));
			modelMatrixLamp = glm::translate(modelMatrixLamp, glm::vec3(0.0, 10.48, 0.0));
			modelMatrixLamp = glm::scale(modelMatrixLamp, glm::vec3(0.5));
			shaderMulLighting.setVectorFloat3(
				"pointLights[" + std::to_string(i) + "].position", 
				glm::value_ptr(glm::vec3(modelMatrixLamp[3])));
			shaderMulLighting.setVectorFloat3(
			...
			shaderMulLighting.setFloat(
				"pointLights[" + std::to_string(i) + "].linear", 0.005f);
			shaderMulLighting.setFloat(
				"pointLights[" + std::to_string(i) + "].quadratic", 0.002f);
		...
		//Lampara 2
		for (int i = lamp1Position.size(); i < lamp1Position.size() + lamp2Position.size(); i++){
			glm::mat4 modelMatrixLamp = glm::mat4(1.0);
			modelMatrixLamp = glm::translate(modelMatrixLamp, lamp2Position[i - lamp1Position.size()].first);
			modelMatrixLamp = glm::rotate(
				modelMatrixLamp, glm::radians(lamp2Position[i - lamp1Position.size()].second), glm::vec3(0, 1, 0));
			modelMatrixLamp = glm::translate(modelMatrixLamp, glm::vec3(7.0, 5.0, 0.0));
			modelMatrixLamp = glm::scale(modelMatrixLamp, glm::vec3(0.5));
			shaderMulLighting.setVectorFloat3(
				"pointLights[" + std::to_string(i) + "].position", 
				glm::value_ptr(glm::vec3(modelMatrixLamp[3])));
			...
			shaderMulLighting.setFloat(
				"pointLights[" + std::to_string(i) + "].constant", 1.0f);
			shaderMulLighting.setFloat(
				"pointLights[" + std::to_string(i) + "].linear", 0.01f);
			shaderMulLighting.setFloat(
				"pointLights[" + std::to_string(i) + "].quadratic", 0.005f);
		...
		int offsetLamp3 = lamp1Position.size() + lamp2Position.size();

		for (int i = 0; i < lamp3Position.size(); i++) {
			int lightIdx = offsetLamp3 + i;
			glm::mat4 mMLamp = glm::mat4(1.0);
			mMLamp = glm::translate(mMLamp, lamp3Position[i].first);
			mMLamp = glm::rotate(mMLamp, glm::radians(lamp3Position[i].second), glm::vec3(0, 1, 0));
			mMLamp = glm::translate(mMLamp, glm::vec3(0.0, 5.0, 0.0)); // Altura del foco emisor de luz

			shaderMulLighting.setVectorFloat3("pointLights[" + std::to_string(lightIdx) + "].position", glm::value_ptr(glm::vec3(mMLamp[3])));
			shaderMulLighting.setVectorFloat3("pointLights[" + std::to_string(lightIdx) + "].light.ambient", glm::value_ptr(glm::vec3(0.2, 0.16, 0.01)));
			shaderMulLighting.setVectorFloat3("pointLights[" + std::to_string(lightIdx) + "].light.diffuse", glm::value_ptr(glm::vec3(0.4, 0.32, 0.02)));
			shaderMulLighting.setVectorFloat3("pointLights[" + std::to_string(lightIdx) + "].light.specular", glm::value_ptr(glm::vec3(0.7, 0.68, 0.03)));
			shaderMulLighting.setFloat("pointLights[" + std::to_string(lightIdx) + "].constant", 1.0f);
			shaderMulLighting.setFloat("pointLights[" + std::to_string(lightIdx) + "].linear", 0.009f);
			shaderMulLighting.setFloat("pointLights[" + std::to_string(lightIdx) + "].quadratic", 0.003f);

			shaderTerrain.setVectorFloat3("pointLights[" + std::to_string(lightIdx) + "].position", glm::value_ptr(glm::vec3(mMLamp[3])));
			shaderTerrain.setVectorFloat3("pointLights[" + std::to_string(lightIdx) + "].light.ambient", glm::value_ptr(glm::vec3(0.2, 0.16, 0.01)));
			shaderTerrain.setVectorFloat3("pointLights[" + std::to_string(lightIdx) + "].light.diffuse", glm::value_ptr(glm::vec3(0.4, 0.32, 0.02)));
			shaderTerrain.setVectorFloat3("pointLights[" + std::to_string(lightIdx) + "].light.specular", glm::value_ptr(glm::vec3(0.7, 0.68, 0.03)));
			shaderTerrain.setFloat("pointLights[" + std::to_string(lightIdx) + "].constant", 1.0f);
			shaderTerrain.setFloat("pointLights[" + std::to_string(lightIdx) + "].linear", 0.009f);
			shaderTerrain.setFloat("pointLights[" + std::to_string(lightIdx) + "].quadratic", 0.003f);
		}
		...
		// Render de modelos de luces
		for (int i = 0; i < lamp1Position.size(); i++) {
			lamp1Position[i].first.y = terrain.getHeightTerrain(
				lamp1Position[i].first.x, lamp1Position[i].first.z);
			modelLamp1.setPosition(lamp1Position[i].first);
			modelLamp1.setScale(glm::vec3(0.5,0.5,0.5));
			modelLamp1.setOrientation(glm::vec3(0,lamp1Position[i].second,0));
			modelLamp1.render();
		};

		// Render de modelos de luces
		for (int i = 0; i < lamp2Position.size(); i++) {
			lamp2Position[i].first.y = terrain.getHeightTerrain(
				lamp2Position[i].first.x, lamp2Position[i].first.z);
			modelLamp2.setPosition(lamp2Position[i].first);
			modelLamp2.setScale(glm::vec3(0.5,0.5,0.5));
			modelLamp2.setOrientation(glm::vec3(0,lamp2Position[i].second,0));
			modelLamp2.render();
			modelLamp2Post.setPosition(lamp2Position[i].first);
			modelLamp2Post.setScale(glm::vec3(0.5,0.5,0.5));
			modelLamp2Post.setOrientation(glm::vec3(0,lamp2Position[i].second,0));
			modelLamp2Post.render();
		};

		// Render de modelos de luces
		for (int i = 0; i < lamp3Position.size(); i++) {
			lamp3Position[i].first.y = terrain.getHeightTerrain(
				lamp3Position[i].first.x, lamp3Position[i].first.z);
			modelLamp3.setPosition(lamp3Position[i].first);

			modelLamp3.setScale(glm::vec3(0.5, 0.5, 0.5)); 
			modelLamp3.setOrientation(glm::vec3(0, lamp3Position[i].second, 0));
			modelLamp3.render();
		}
		...
		//Lambo
		glm::vec3 ejeyLambo = glm::normalize(terrain.getNormalTerrain(modelMatrixLambo[3][0], modelMatrixLambo[3][2]));
		glm::vec3 ejexLambo = glm::vec3(modelMatrixLambo[0]);
		glm::vec3 ejezLambo = glm::normalize(glm::cross(ejexLambo, ejeyLambo));
		ejexLambo = glm::normalize(glm::cross(ejeyLambo, ejezLambo));

		modelMatrixLambo[0] = glm::vec4(ejexLambo, 0.0);
		modelMatrixLambo[1] = glm::vec4(ejeyLambo, 0.0);
		modelMatrixLambo[2] = glm::vec4(ejezLambo, 0.0);

		modelMatrixLambo[3][1] = terrain.getHeightTerrain(modelMatrixLambo[3][0], modelMatrixLambo[3][2]);
		glm::mat4 modelMatrixLamboRender = glm::scale(modelMatrixLambo, glm::vec3(1.3, 1.3, 1.3));
		modelLambo.render(modelMatrixLamboRender);
